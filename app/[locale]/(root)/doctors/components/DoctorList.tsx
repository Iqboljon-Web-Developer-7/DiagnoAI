...
export function DoctorList({ doctors, user }: DoctorListProps) {
    ...
    const bookedTimes = freeTimes?.booked_times?.map(time => parseInt(time.split(':')[0])) ?? [];
                            ...
                            <div className="flex-1">
                                <div className="flex items-start justify-between">
                                    <div className="w-full">
                                        <h3 className="text-xl font-bold text-gray-900 mb-1 underline" 
                                            onClick={() => router.push(`/doctors/${doctor.id}`)}>
                                            {doctor.name}
                                        </h3>
                                        <p className="text-blue-600 font-medium mb-2">{doctor.translations?.field}</p>
                                        <p className="text-gray-600 text-sm mb-3">Hospital: {doctor.hospital.name}</p>
