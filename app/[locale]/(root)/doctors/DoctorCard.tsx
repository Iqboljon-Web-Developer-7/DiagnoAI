...
const DoctorCard = memo(({ doctor, user, index }: DoctorCardProps) => {
    ...
    return (
        <motion.div
            ...
            animate="visible"
        >
            <Card className="group relative overflow-hidden backdrop-blur-sm bg-white/90 dark:bg-gray-900/90 hover:bg-white dark:hover:bg-gray-900 hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 cursor-pointer border border-gray-200/50 dark:border-gray-700/50 hover:border-blue-200 dark:hover:border-blue-400">
                <div className="absolute inset-0 bg-gradient-to-br from-blue-50/50 dark:from-blue-900/50 via-transparent to-purple-50/30 dark:to-purple-900/30 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

                {doctor.prize && parseInt(doctor.prize.replace(/\D/g, '')) > 500000 && (
                    <div className="absolute top-2 right-2 sm:top-4 sm:right-4 z-10">
                        <div className="bg-gradient-to-r from-amber-400 to-yellow-500 text-white text-[10px] sm:text-xs font-bold px-2 sm:px-3 py-0.5 sm:py-1 rounded-full flex items-center gap-1 shadow-lg">
                ...
                <CardContent className="p-2 py-4 sm:p-6 lg:p-8 relative z-10">
                    <div className="flex items-start gap-4 sm:gap-6 flex-col lg:flex-row">
                        ...
                        >
                            <div className="relative">
                                <Zoom>
                                    <div className="absolute inset-0 bg-gradient-to-br from-green-100 to-green-500 dark:from-green-900 dark:to-green-700 rounded-xl sm:rounded-2xl blur-lg opacity-20 group-hover:opacity-40 transition-opacity duration-500" />
                                    <Image
                                        width={500}
                                        height={500}
                                        src={`https://api.diagnoai.uz${doctor.image}`}
                                        alt={doctor.name}
                                        className="relative w-20 h-20 sm:w-24 sm:h-24 lg:w-32 lg:h-32 rounded-xl sm:rounded-2xl object-cover ring-2 sm:ring-4 ring-white dark:ring-gray-900 shadow-lg group-hover:shadow-xl transition-all duration-300"
                                        loading="lazy"
                                    />
                        ...
                        <div className="flex-1 space-y-2 sm:space-y-4 text-center lg:text-left w-full">
                            <div>
                                ...
                                <div className="flex items-center justify-center lg:justify-start gap-1 sm:gap-2 mb-2 sm:mb-3">
                                    <span className="bg-gradient-to-r from-blue-600 to-green-600 dark:from-blue-700 dark:to-green-700 text-white px-2 sm:px-4 py-0.5 sm:py-1 rounded-full text-xs sm:text-sm font-semibold">
                                ...
                                <Link href={`/hospitals/${doctor?.hospital?.id}`} className="flex items-center justify-center lg:justify-start gap-1 sm:gap-2 text-gray-600 dark:text-gray-300 mb-2 sm:mb-4 hover:underline">
                            ...
                            <div className="flex items-center justify-between gap-2 sm:gap-4 border-t border-gray-100 dark:border-gray-800 flex-wrap">
                                ...
                                {user?.role !== 'clinic' && (
                                    <motion.button
                                        ...
                                        className="mx-auto shrink-0 md:mx-0 relative overflow-hidden bg-gradient-to-r from-blue-600 to-green-600 dark:from-blue-700 dark:to-green-700 hover:from-blue-700 hover:to-green-700 dark:hover:from-blue-800 dark:hover:to-green-800 text-white font-semibold py-2 sm:py-3 px-4 sm:px-6 rounded-lg sm:rounded-xl transition-all duration-300 shadow-lg hover:shadow-xl group/btn"
                                        whileHover={{ scale: 1.05, y: -2 }}
                                        whileTap={{ scale: 0.98 }}
                                        transition={{ type: "spring", stiffness: 300, damping: 20 }}
                                    >
                                        <div className="absolute inset-0 bg-gradient-to-r from-green-700 to-green-700 dark:from-green-800 dark:to-green-800 opacity-0 group-hover/btn:opacity-100 transition-opacity duration-300" />
                                        ...
                                        <div className="absolute inset-0 -top-full bg-gradient-to-b from-transparent via-white/20 dark:via-gray-900/20 to-transparent group-hover/btn:top-full transition-all duration-700" />
                                    </motion.button>
                                )}
                            </div>
                        </div>
